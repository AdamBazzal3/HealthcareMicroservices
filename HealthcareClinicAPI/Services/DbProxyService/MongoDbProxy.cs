...
using HealthcareClinicAPI.Settings.DbSettings;
...
namespace HealthcareClinicAPI.Services.DbProxyService
{
    public class MongoDbProxy<T> : IDbProxy<T> where T : class
    {
        ...
        {
            var mongoClient = new MongoClient(Environment.GetEnvironmentVariable("ConnectionString") ?? "mongodb://localhost:27017");

            var mongoDatabase = mongoClient.GetDatabase(Environment.GetEnvironmentVariable("DatabaseName") ?? "ClinicDatabase");
        ...
        public async Task<List<T>> GetAsync(FilterDefinition<T> filter, ProjectionDefinition<T> projection)
        =>    await _instancesCollection.Find(filter).ToListAsync();
