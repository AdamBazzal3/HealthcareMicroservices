...
public class ConsulServiceRegistration : IHostedService
{
    ...
    {
        bool success = int.TryParse(Environment.GetEnvironmentVariable("CLINIC_API_PORT"), out int port);
        if (!success) return;
        // Service registration details
        var registration = new AgentServiceRegistration()
        {
            ID = $"HealthcareClinicAPI-{Guid.NewGuid()}",
            Name = AppConstants.CLINICAL_SERVICE_NAME.ToString(),
            Address = "HealthcareClinicAPI", // Replace with your service's actual address
